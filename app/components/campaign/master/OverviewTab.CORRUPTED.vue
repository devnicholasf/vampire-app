				...
				<div class="df-card">
					...
					<div class="relative z-10">
						...
						<div class="flex items-center gap-2 mb-4 pb-3 border-b border-df-border-red/30">
							...
							<h5 class="text-sm font-bold text-df-gold uppercase tracking-wider">Líder do Domínio</h5>
						</div>

						<!-- Loading -->
						<div v-if="loadingPolitics" class="flex items-center justify-center py-8">
							<svg class="w-5 h-5 animate-spin text-df-muted" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
								<polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 11-2.12-9.36L23 10"/>
							</svg>
						</div>

						<!-- Leader found -->
						<div v-else-if="domainLeader" class="flex flex-col items-center text-center py-2">
							<!-- Avatar -->
							<div class="relative w-20 h-20 mb-3">
								<img 
									v-if="domainLeader.avatar" 
									:src="domainLeader.avatar" 
									:alt="domainLeader.name"
									class="w-full h-full rounded-full object-cover border-2 border-df-gold/50"
								/>
								<div v-else class="w-full h-full rounded-full bg-df-deep border-2 border-df-gold/50 flex items-center justify-center">
									<svg class="w-8 h-8 text-df-muted" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
							...
							<!-- Name -->
							<h6 class="text-df-silver font-semibold text-base mb-1">{{ domainLeader.name }}</h6>
							
							<!-- Title -->
							<p class="text-df-gold text-xs font-bold uppercase tracking-wide mb-2">{{ domainLeader.title }}</p>
							
							<!-- Clan -->
							<p v-if="domainLeader.clan" class="text-df-muted text-xs">{{ domainLeader.clan }}</p>
						</div>

						<!-- No leader -->
						<div v-else class="py-8 text-center">
							<svg class="w-8 h-8 mx-auto mb-2 text-df-muted/40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
								<path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/>
							</svg>
							<p class="text-df-muted text-xs italic">Nenhum líder definido</p>
						</div>
				...
				<div class="df-card">
					...
					<div class="relative z-10">
						...
						<div class="flex items-center gap-2 mb-4 pb-3 border-b border-df-border-red/30">
							...
							<h5 class="text-sm font-bold text-df-gold uppercase tracking-wider">Territórios</h5>
						</div>

						<!-- Loading -->
						<div v-if="loadingPolitics" class="flex items-center justify-center py-6">
							<svg class="w-5 h-5 animate-spin text-df-muted" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
								<polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 11-2.12-9.36L23 10"/>
							</svg>
						</div>
						...
						<!-- Territories found -->
						<div v-else-if="territoryZones.length > 0" class="space-y-3">
							<div v-for="zone in territoryZones" :key="zone.name" class="flex items-center gap-2">
								...
								<div class="flex-1 min-w-0">
									<p class="text-df-silver text-xs font-medium truncate">{{ zone.name }}</p>
									<p class="text-df-muted text-[10px] uppercase tracking-wide">{{ getZoneStatusLabel(zone.status) }}</p>
								</div>

								<!-- Influence indicator (dominant faction) -->
								<div v-if="zone.influences && zone.influences.length > 0 && zone.influences[0]" class="flex-shrink-0">
									<div 
										class="w-2 h-6 rounded-sm" 
										:style="{ backgroundColor: zone.influences[0].color }"
										:title="zone.influences[0].faction"
									></div>
								</div>
							</div>

							<!-- Summary bar -->
							<div class="pt-3 mt-3 border-t border-df-border-silver/20">
								<div class="flex items-center justify-between text-[10px] text-df-muted mb-1.5">
									<span>Status Geral</span>
									<span>{{ territoryZones.length }} {{ territoryZones.length === 1 ? 'zona' : 'zonas' }}</span>
								</div>
								<div class="flex gap-1 h-2 rounded-full overflow-hidden bg-df-deep">
									<div 
										v-if="territorySummary.stable > 0"
										class="bg-green-500/70"
										:style="{ width: `${(territorySummary.stable / territoryZones.length) * 100}%` }"
										:title="`${territorySummary.stable} estável(is)`"
									></div>
									<div 
										v-if="territorySummary.contested > 0"
										class="bg-yellow-500/70"
										:style="{ width: `${(territorySummary.contested / territoryZones.length) * 100}%` }"
										:title="`${territorySummary.contested} contestado(s)`"
									></div>
									<div 
										v-if="territorySummary.war > 0"
										class="bg-red-500/70"
										:style="{ width: `${(territorySummary.war / territoryZones.length) * 100}%` }"
										:title="`${territorySummary.war} em guerra`"
									></div>
									<div 
										v-if="territorySummary.abandoned > 0"
										class="bg-gray-500/70"
										:style="{ width: `${(territorySummary.abandoned / territoryZones.length) * 100}%` }"
										:title="`${territorySummary.abandoned} abandonado(s)`"
									></div>
								</div>
							</div>
						</div>
						...
						<div v-else class="py-6 text-center">
							...
							<p class="text-df-muted text-xs italic">Nenhum território mapeado</p>
						</div>
...
interface TerritoryZone {
	...
	points: number[][]
	influences: { faction: string; percent: number; color: string }[]
}
...
// ── Politics & Territory data ──
const politics = ref<PoliticsData>({
	government: [],
	factions: [],
	relations: [],
	territories: [],
...
// ── Computed: Domain Leader ──
const domainLeader = computed(() => {
	const leaderTitles = ['Príncipe', 'Príncipe (Camarilla)', 'Barão', 'Barão (Anarquistas)']
	const leaderRole = politics.value.government.find(role => 
		leaderTitles.some(title => role.title.toLowerCase().includes(title.toLowerCase()))
	)
	
	if (!leaderRole?.npcId) return null
	
	const npc = npcs.value.find(n => n.id === leaderRole.npcId)
	if (!npc) return null
...
// ── Computed: Territory Zones ──
const territoryZones = computed(() => politics.value.territoryZones || [])

// ── Computed: Territory Summary ──
const territorySummary = computed(() => {
	...
	territoryZones.value.forEach(zone => {
		if (zone.status in summary) {
			summary[zone.status]++
		}
	})
...
// ── Helpers for territory status ──
const statusConfig: Record<string, { label: string; color: string }> = {
	stable:     { label: 'Estável',     color: '#22c55e' },
	contested:  { label: 'Contestado',  color: '#eab308' },
	war:        { label: 'Em Guerra',   color: '#ef4444' },
	abandoned:  { label: 'Abandonado',  color: '#6b7280' }
}

const getZoneStatusColor = (status: string) => {
	return statusConfig[status]?.color || '#22c55e'
}

const getZoneStatusLabel = (status: string) => {
	return statusConfig[status]?.label || 'Estável'
}
...
// ── Load Politics & NPCs for widgets ──
const loadPoliticsData = async () => {
	...
	try {
		// Load politics data
		const { data: politicsData } = await supabase
			.from('campaigns')
			.select('politics')
			.eq('id', props.campaignId)
			.single()
		
		if (politicsData?.politics) {
			politics.value = politicsData.politics
		}
		
		// Load NPCs
		const { data: npcsData } = await supabase
			.from('campaign_npcs')
			.select('id, name, clan, avatar, sect')
			.eq('campaign_id', props.campaignId)
		...
		console.error('Erro ao carregar dados políticos:', err)
	} finally {
