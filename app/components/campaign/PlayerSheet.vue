<template>
  ...
  >
    <div class="relative w-full max-w-7xl max-h-[98vh]">
      ...
      <!-- Header editorial no estilo Editar NPC -->
      <div class="mb-0 sticky top-0 z-10 pb-2 pt-0 df-header-premium">
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1 min-w-0">
            <h3 class="text-xl font-bold text-white tracking-wide">Editar Personagem</h3>
            <p class="text-xs text-df-muted italic">Cada vampiro possui fome, ambicao e segredos.</p>
          </div>
          <button
            type="button"
            @click="handleClose"
              ...
              <div v-for="attr in physicalAttributes" :key="attr.key" class="df-attribute-row">
                ...
                <div class="df-attribute-dots">
                  <button
                    ...
                    :disabled="!canEdit"
                    :class="[
                      'df-dot df-dot-md df-attribute-dot',
        ...
        <div class="df-card">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div class="space-y-3 md:border-l md:border-df-border-red/35 md:pl-4">
              <div v-for="skill in skills" :key="`skills-${skill.key}`" class="df-skill-row">
                <div class="df-skill-meta">
                    <label class="df-attr-label df-skill-name">{{ skill.name }}</label>
                    <span
      ...
      <div class="df-footer-premium df-footer-actions">
        <button
          type="button"
...
import {
  ...
  getDisciplineIconPath,
  knowledges,
  mentalAttributes,
  physicalAttributes,
  predatorDescriptions,
  sectOptions,
  skills,
  socialAttributes,
  talents,
  vampireDisciplines
} from '~/components/campaign/player-sheet/constants'
import {
...
import { createInitialPlayerSheetData } from '~/components/campaign/player-sheet/sheetDataFactory'
import {
  findAdvantageItem,
  getNameOptions as getAdvantageNameOptions,
  getSubcategoryOptions as getSubcategoryOptionsByGeneration,
  normalizeCategoryKey,
  shouldShowTypeDropdown
} from '~/components/campaign/player-sheet/advantages'
import {
  getCustomSpecialtyPlaceholder,
  getInitials,
  isValidAdvantage
} from '~/components/campaign/player-sheet/uiHelpers'
...
const getPredatorSummary = (predatorName: string) => {
  ...
  return predatorTypeOptions.value.find(p => p.name === predatorName)?.description || predatorDescriptions[predatorName] || ''
}
...
const sheetData = ref(createInitialPlayerSheetData(props.player, clanBanes))
...
const getSubcategoryOptions = (category: string) => {
  return getSubcategoryOptionsByGeneration(category, Number(sheetData.value.generation) || 0)
}

const getNameOptions = (adv: any) => {
  return getAdvantageNameOptions(adv, vantagensDados)
}
...
const onNameChange = (index: number) => {
  ...
  // Para Méritos e Defeitos: buscar pontos e max do objeto de dados
    ...
    const selectedItem = findAdvantageItem(adv, vantagensDados)
    ...
    if (selectedItem) {
      ...
      adv.fixo = false
      adv.maxLevel = selectedItem.max || 5
    }
  }
...
.df-identity-group-clan :deep(.df-clan-trigger .text-xs) {
  margin-top: 2px;
  line-height: 1.15;
}
...
.df-identity-group-generation :deep(label) {
  margin: 0;
}
...
@media (min-width: 1024px) {
  ...
  .df-identity-span-name {
    grid-column: span 4;
  }

  .df-identity-span-concept {
    grid-column: span 3;
  }

  .df-identity-span-clan {
    grid-column: span 5;
  }
...
select.df-input {
  appearance: none;
  background-image:
    linear-gradient(45deg, transparent 50%, var(--df-text-gold) 50%),
    linear-gradient(135deg, var(--df-text-gold) 50%, transparent 50%);
  background-position:
